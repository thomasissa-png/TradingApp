...
> **Phase v3 en cours : mode shadow (état 2026-06-19, Session 8)** — **1508 tests**. Backtest quant = **NO-GO, écarté comme jalon** (validation = shadow réel uniquement). **Refonte « 5 rapports/jour » LIVE** + S8 : mesure des continus depuis l'**émission 7h** (fix L027), briefing refondu + **« raison par cellule » auditée 10/10**, **3 instrumentations SHADOW** (persistance/news-par-horizon/rumeur — cutovers en attente de ~2 sem. de mesure), 5 critères météo symétriques, garde-fou férié `is_any_market_open`, fix parser news. **WIN RATE ONLY.** Prochaine étape : laisser les shadows accumuler → trancher les cutovers sur preuve + Ticket C ~23/06 + question produit « rôle des news ». Détail : `project-context.md` (source de vérité courante).
