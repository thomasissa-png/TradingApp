...
| 2026-06-02 | orchestration + 3 experts + @fullstack | **Audit trio du bulletin + correctif FRED-429 + fix cron TZ** | **Trio** (Analyst/News Trader/Spéculateur) sur bulletin 12h29 : **AJUSTER 6/10** unanime (`v3/audit/bulletin-*.md` + `coherence-bulletin-2026-06-02.md`). Noyau sain : Or SHORT, Pétrole/Argent LONG. **Audit des recos avant d'appliquer** : le « 42% INSUFFISANT » = **FRED 429** (rate-limit → `DFII10`/TIPS morts), PAS un seuil ni un trou d'archi → **corrigé** (retry/backoff + throttle FRED, commit `a03cd98`, +6 tests). Coin-flips Nasdaq/VIX 24h : **gardés flag-only** (décision Thomas — Lot 4b différé maintenu). Cron : `CRON_TZ` inopérant sur Vixie/Ubuntu (midi 02/06 raté) → bascule **cron horaire `0 * * * *` + garde heure-Paris dans le script** (robuste DST). **FRED vérifié en prod** (run #28) : `fred_dead:DFII10` disparu, TIPS revient. **Généralisé** : helper partagé `http_retry.py` (429/5xx + `Retry-After` + throttle par bucket) → GNews/NewsAPI + UA Chrome pour le 403 mining_com (commit `e28f40f`, +16 tests, 632 verts). Reste non traité (low prio) : `http_get_json` CFTC/EIA/Open-Meteo (point 5, wrapper requis) + `fred_spread_thin` DGS10-DE (série allemande mensuelle, limite de donnée pas rate-limit). |
