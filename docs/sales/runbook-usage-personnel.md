...
| **8h40:15-8h44** | Bot (background) | Pipeline scoring : Twelve Data delta cache → backtester → ScoringEngine (D1-D6 + SC1-SC7) → Claude Sonnet 4.6 (tool use) → SQLite INSERT `signals` → ping healthchecks.io | Aucun (Thomas RER ou trajet) | Si timeout Sonnet 25s → fallback Haiku 4.5 (cf. ai-architecture v1.2) |
