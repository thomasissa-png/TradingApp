...
| 5 | Tag modèle Anthropic exact verrouillé (refus alias `-latest`) | `Config.from_env()` rejette `*-latest` et `*-newest` (testé `tests/test_config.py`). Secrets Replit `ANTHROPIC_MODEL_LIVE=claude-sonnet-4-5-20250929`. | **PASS** — règle L002 garde-fou actif |
