...
## Addendum — 2026-07-01 : Ticket C — frein des capteurs non-tendance à contre-sens du 24h (cutover EUR/USD)

**Défaut corrigé (preuve du jour, GO fondateur 01/07).** Le **EUR/USD 24h SHORT** du 01/07 était porté par des capteurs non-tendance à contre-sens des tendances courtes positives : `usd_jpy_proxy_risk` (effet **−2.68**), écart de taux 2 ans (−0.94), dollar 20j (−0.88) — **CONTRE** les tendances 3j (+1.47) et 7j (+1.46). Pattern identique au piège « taux réels contre l'or » de juin (le capteur lourd bat la tendance sur le 24h). La pertinence 24h de `usd_jpy_proxy_risk` (**0.7**) dominait celle des tendances courtes (3j=1.0, 7j=0.5) alors qu'un signal de risque n'a pas à piloter le trade du JOUR contre la tendance.

**Correctif (pertinence 24h, périmètre validé EUR/USD + CAC 40).** La pertinence **24h** de `usd_jpy_proxy_risk` passe de **0.7 → 0.1** (7j/1m INCHANGÉS : 0.8 / 0.5) → l'horizon 24h est désormais dominé par les tendances courtes. Effet 01/07 : score 24h **−1.35 → +0.94 (SHORT → LONG)**. Les autres capteurs structurels EUR/USD (taux 2y/10y, dxy, spread OAT-Bund, COT, FedWatch) étaient **déjà à 0.1** sur le 24h (reweight 30/06) — non touchés. **CAC 40 : AUCUN changement** — son seul capteur structurel (`spread_oat_bund_10y`) est déjà à pertinence 24h 0.1 (< tendances courtes) et le 24h CAC est déjà **dominé par `momentum_prix_3j`** (top driver, dans le sens du call) ; aucun capteur structurel n'y a une pertinence 24h ≥ celle des tendances courtes → rien à réduire (zéro modification, pas de cutover CAC, cf. handoff).

**Impact kill-criterion.** La **formule** (WR tradable < seuil sur N suffisant, 24h-only) reste **inchangée**. Seul **EUR/USD** voit son signal 24h changer → `ref_changed` de `EUR=X` avancé au **2026-07-02** (`ref-changed.json`, append-only, justifié). Reset N=0 pour EUR/USD au 1er bulletin sous la nouvelle pertinence. Les 14 autres cellules conservent leur historique depuis le 30/06. Test : `tests/test_regles_selection_2026_07_01.py` (valeurs de pertinence de la fiche + replay numérique du flip SHORT → LONG).
