...
def init_database(data_dir: Path | str) -> Path:
    ...
    Inclut la migration ALTER TABLE strategy_state ADD COLUMN mode (Phase 2c-2).
    """
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for ddl in get_all_ddls():
            conn.execute(ddl)
        # Migration idempotente Phase 2c-2
...
def insert_trade(
    ...
    cursor = conn.execute(
        ...
        INSERT INTO trades (
            ...
            pnl_net_avant_pfu, mae, mfe, exit_date
        ) VALUES (?, 1, ?, 0.99, 0.99, ?, ?, ?, ?)
        """,
        (
            signal_id,
...
def insert_strategy_pause(
    ...
    Validation amont : start <= end, durée <= 30 jours.
    Désactive toute pause active précédente (status='cancelled').
    """
    ...
    # Désactiver pauses actives existantes
    conn.execute(
