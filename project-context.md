...
- ⚠️ **À VÉRIFIER (priorité 1, run frais)** : sur données sandbox vieilles de 3 j, les 12 actifs ressortent en `INSUFFISANT` (couverture 0-35%). Probable artefact de données périmées, mais valider sur le run frais de 7h que `COVERAGE_MIN=0.40` est bien calibré (sinon recalibrer ou vérifier que le pipeline remplit assez de critères).
